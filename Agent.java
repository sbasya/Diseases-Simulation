        ...
        for (int i = 0; i < inbox.size(); i++) {
            ...
            if (message != null) {
                switch (message) {
                    ...
                    case BECOME_DEAD:
                        state = State.DEAD;
                        StatisticsGraph.incDead();
                        break;
                    case BECOME_IMMUNE:
                        state = State.IMMUNE;
                        StatisticsGraph.incImm();
                        break;
                    case BECOME_ASYMPTOMATIC:
                        state = State.ASYMPTOMATIC;
                        StatisticsGraph.incAsym();
                        break;
                    case BECOME_SICK:
                        state = State.SICK;
                        StatisticsGraph.incSick();
                        break;
                    default: stateChange = false;
                }
                if(stateChange) Platform.runLater(() -> SimulationHistory.addToHistory(agentNum,this.toString()));
            }
        }
    }
